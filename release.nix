...
{ config ? {}, overlays ? [], nixpkgs ? <nixpkgs>
...
let
  jobs = import (builtins.toString nixpkgs + "/" + release) {
    inherit officialRelease supportedSystems scrubJobs nixpkgsArgs;
  };
in
  { inherit (jobs) nixpkgs-overlay; }
